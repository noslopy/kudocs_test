class PageVisitsAnalyzer
    ...
    def initialize(filepath)
        raise `Page visits log file not found on given path: "#{filepath}"` unless File.file?(filepath)
    ...
    def pages_by_unique_visitors
        sorted_list = @page_visit_analytics.sort_by {|_key, value| -value[:unique_visitors].count}
        sorted_list.map{ |p, v| [p, v[:unique_visitors].count] }
    end

    private

    def scrape_visit_logs log_filepath
        page_visit_data = {}
        visit_logs = File.open(log_filepath)
